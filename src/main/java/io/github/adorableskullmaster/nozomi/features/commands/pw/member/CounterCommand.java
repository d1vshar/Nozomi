...
public class CounterCommand extends MemberPoliticsAndWarCommand {
  ...
  protected void execute(CommandEvent commandEvent) {
    ...
    try {
        List<AllianceProfile> discordServer = new AllianceProfileRepository().findAllianceProfilesByServerId(commandEvent.getGuild().getIdLong());
      commandEvent.async(() -> {
        ...
        if (!commandEvent.getArgs().trim().isEmpty() && commandEvent.getArgs().trim().split(" ").length != 1) {
          ...
          if (Utility.isNumber(args)) {
            id = Integer.parseInt(args);
              commandEvent.reply(getMessage(id, discordServer.get(0).getAaId()));
    ...
    public Message getMessage(int targetId, int aaId) {
    MessageBuilder messageBuilder = new MessageBuilder();
    Nation targetNation = politicsAndWar.getNation(targetId);
        messageBuilder.setEmbed(getCounters(targetId, aaId).build());
